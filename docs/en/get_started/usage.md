...
- `--policy-loss-type`: Selects the policy-gradient objective. The default `vanilla` keeps the existing PPO-style surrogate. Set `--policy-loss-type future_kl` to enable the FIPO future-KL influenced objective, which weights token advantages by discounted future log-ratio drift. The main FIPO knobs are `--future-kl-decay-rate`, `--future-kl-start`, `--future-kl-window`, `--future-kl-clip-ratio`, and `--future-kl-clip-high-only`. This mode uses token-level policy ratios and is not compatible with `--advantage-estimator gspo`.
