...
- `--policy-loss-type`：选择 policy gradient 目标函数。默认 `vanilla` 保持现有 PPO surrogate；设置 `--policy-loss-type future_kl` 可以启用 FIPO 的 future-KL influenced objective，用折扣后的未来 log-ratio drift 对 token advantage 加权。主要参数包括 `--future-kl-decay-rate`、`--future-kl-start`、`--future-kl-window`、`--future-kl-clip-ratio` 与 `--future-kl-clip-high-only`。该模式使用 token-level policy ratio，不兼容 `--advantage-estimator gspo`。
