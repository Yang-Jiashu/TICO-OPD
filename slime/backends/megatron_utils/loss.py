...
def _get_compression_budget(args: Namespace, response_len: int) -> int:
    if args.compression_length_budget > 0:
        return min(args.compression_length_budget, response_len)
    if args.compression_length_budget_ratio > 0:
        return max(1, min(response_len, int(response_len * args.compression_length_budget_ratio)))
    return response_len


def apply_compression_opd_to_advantages(
    args: Namespace,
    rollout_data: RolloutBatch,
    advantages: list[torch.Tensor],
    ...
    """Apply compression-aware low-importance token shaping.

    This is intentionally selective: high-importance tokens are preserved, while
    low-importance continuation tokens beyond the budget/coverage point receive
    a negative advantage.  Importance is approximated by future accumulated
    teacher/student log-prob divergence when teacher log-probs are available.
    """
    if args.compression_advantage_coef == 0 and args.compression_eos_coef == 0:
        return
    ...
    truncated = rollout_data.get("truncated", [0] * len(advantages))
    ...
    for i, adv in enumerate(advantages):
        ...
        if teacher_log_probs is not None:
            token_signal = (student_log_probs[i] - teacher_log_probs[i]).abs()
        elif kl is not None:
            token_signal = kl[i].abs()
        else:
            token_signal = torch.ones_like(adv)

        if mpu.get_context_parallel_world_size() > 1:
            full_signal = all_gather_with_cp(token_signal, total_len, response_len)
        else:
            full_signal = token_signal

        full_mask = loss_masks[i].to(device=full_signal.device)
        importance = compute_future_token_importance(
            full_signal,
            full_mask,
            decay_rate=args.compression_importance_decay_rate,
            start=args.compression_importance_start,
            window=args.compression_importance_window,
            average=args.compression_importance_average,
            temperature=args.compression_importance_temperature,
        )

        budget = _get_compression_budget(args, response_len)
        positions = torch.arange(response_len, device=importance.device)
        valid_importance = importance * full_mask.to(importance.dtype)
        total_importance = torch.clamp_min(valid_importance.sum(), 1e-6)
        coverage = torch.cumsum(valid_importance, dim=0) / total_importance
        ...
        penalty = (
            ...
            * (1.0 - importance)
            * compression_zone.to(importance.dtype)
            * full_mask.to(importance.dtype)
        )

        eos_bonus = torch.zeros_like(penalty)
        if args.compression_eos_coef != 0 and full_mask.sum() > 0:
            valid_indices = full_mask.nonzero(as_tuple=True)[0]
            last_idx = valid_indices[-1]
            if truncated[i]:
                eos_bonus[last_idx] = -args.compression_eos_coef
            elif coverage[last_idx] >= args.compression_coverage_threshold:
                eos_bonus[last_idx] = args.compression_eos_coef

        delta = eos_bonus - penalty
        if mpu.get_context_parallel_world_size() > 1:
        ...
        if returns is advantages:
            advantages[i] = new_advantage
        else:
            advantages[i] = new_advantage
...
def policy_loss_function(
    ...
    if args.policy_loss_type == "future_kl" and args.advantage_estimator == "gspo":
        raise ValueError("future_kl policy loss uses token-level policy ratios and is not compatible with GSPO KL.")
    ...
    if args.advantage_estimator == "gspo":
        ...
        ppo_kl_list = [
            ...
            for old_log_prob, log_prob in zip(old_log_probs_list, log_probs_list, strict=False)
        ]
        old_log_probs = torch.cat(old_log_probs_list, dim=0)
        log_probs = torch.cat(log_probs_list, dim=0)
        ppo_kl = torch.cat(ppo_kl_list, dim=0)

    future_kl_metrics = None
    if args.policy_loss_type == "future_kl":
        eps_clip_c = args.eps_clip_c if args.eps_clip_c is not None else 3.0
        pg_loss, pg_clipfrac, ppo_kl, future_kl_metrics = compute_future_kl_policy_loss(
            local_ppo_kl=ppo_kl_list,
            ...
            future_kl_decay_rate=args.future_kl_decay_rate,
            future_kl_start=args.future_kl_start,
            future_kl_window=args.future_kl_window,
            future_kl_average=args.future_kl_average,
            future_kl_clip_ratio=args.future_kl_clip_ratio,
            future_kl_clip_high_only=args.future_kl_clip_high_only,
            future_kl_safety_threshold=args.future_kl_safety_threshold,
            qkv_format=args.qkv_format,
            max_seq_lens=max_seq_lens,
        )
    else:
    ...
    if future_kl_metrics is not None:
        future_kl_metrics = {key: sum_of_sample_mean(value) for key, value in future_kl_metrics.items()}
    ...
    if future_kl_metrics is not None:
        for key, value in future_kl_metrics.items():
