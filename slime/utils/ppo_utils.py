...
def _discounted_future_sum(
    ...
    *,
    ...
    if start not in {"include_current", "exclude_current"}:
        raise ValueError(f"Unknown future KL start mode: {start}")

    response_len = values.size(0)
    future = torch.zeros_like(values)
    normalizer = torch.zeros_like(values) if average else None
    offset = 0 if start == "include_current" else 1
    masked_values = values * mask.to(values.dtype)

    if window < 0:
        running_value = torch.zeros((), dtype=values.dtype, device=values.device)
        running_weight = torch.zeros((), dtype=values.dtype, device=values.device) if average else None
        for i in reversed(range(response_len)):
            if start == "include_current":
                running_value = masked_values[i] + gamma * running_value
                future[i] = running_value
                if average:
                    running_weight = mask[i].to(values.dtype) + gamma * running_weight
                    normalizer[i] = running_weight
            else:
                future[i] = running_value
                if average:
                    normalizer[i] = running_weight
        ...
        if average:
            future = future / torch.clamp_min(normalizer, 1.0)
        return future

    for i in range(response_len):
        j_start = i + offset
        if j_start >= response_len:
            continue
        j_end = response_len if window < 0 else min(response_len, i + offset + window)
        if j_end <= j_start:
            continue

        distance = torch.arange(j_start - i, j_end - i, device=values.device, dtype=values.dtype)
        if start == "exclude_current":
            distance = distance - 1
        weights = torch.pow(torch.as_tensor(gamma, dtype=values.dtype, device=values.device), distance)
        valid_weights = weights * mask[j_start:j_end].to(values.dtype)
        future[i] = (values[j_start:j_end] * valid_weights).sum()
        if average:
            normalizer[i] = valid_weights.sum()
...
def normalize_token_importance(
    raw_importance: torch.Tensor,
    mask: torch.Tensor,
    *,
    temperature: float = 1.0,
) -> torch.Tensor:
    """Normalize a non-negative token importance signal to [0, 1]."""
    if temperature <= 0:
        raise ValueError(f"importance temperature must be positive, got {temperature}.")

    mask_bool = mask.bool()
    importance = torch.clamp(raw_importance.float(), min=0.0)
    if temperature != 1.0:
        importance = torch.pow(importance, 1.0 / temperature)

    valid = importance[mask_bool]
    if valid.numel() == 0:
    ...
    if torch.isclose(max_val, min_val):
        ...
        normalized = (importance - min_val) / torch.clamp_min(max_val - min_val, 1e-6)
    return normalized * mask.to(normalized.dtype)


def compute_future_token_importance(
    token_signal: torch.Tensor,
    mask: torch.Tensor,
    *,
    decay_rate: float,
    start: str,
    ...
    """Turn a local token signal into normalized future-trajectory importance."""
    if decay_rate <= 0:
        raise ValueError(f"decay_rate must be positive, got {decay_rate}.")

    gamma = 2 ** (-1.0 / decay_rate)
    future_signal = _discounted_future_sum(
        torch.clamp(token_signal.float(), min=0.0),
        mask,
        gamma=gamma,
        start=start,
        window=window,
    ...
    return normalize_token_importance(future_signal, mask, temperature=temperature)


def compute_future_kl_policy_loss(
    local_ppo_kl: list[torch.Tensor],
    ...
    *,
    ...
    eps_clip_high: float,
    eps_clip_c: float,
    future_kl_decay_rate: float,
    future_kl_start: str,
    future_kl_window: int,
    future_kl_average: bool,
    future_kl_clip_ratio: float,
    future_kl_clip_high_only: bool,
    future_kl_safety_threshold: float,
    qkv_format: str = "thd",
    max_seq_lens: list[int] | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
    """Compute FIPO-style future-KL influenced PPO loss.

    FIPO defines the influence signal from future token log-ratio drift.  In
    slime's notation ``ppo_kl = old_log_prob - log_prob``, so the FIPO
    ``negative_approx_kl`` term is ``-ppo_kl``.
    """
    if eps_clip_c <= 1.0:
        raise ValueError(f"eps_clip_c must be greater than 1.0 for future_kl, got {eps_clip_c}.")
    if future_kl_decay_rate <= 0:
        raise ValueError(f"future_kl_decay_rate must be positive, got {future_kl_decay_rate}.")
    if future_kl_clip_ratio < 0:
        raise ValueError(f"future_kl_clip_ratio must be non-negative, got {future_kl_clip_ratio}.")
    ...
    try:
        ...
        cp_size = 1
    gamma = 2 ** (-1.0 / future_kl_decay_rate)

    pg_loss_chunks: list[torch.Tensor] = []
    pg_clipfrac_chunks: list[torch.Tensor] = []
    future_kl_chunks: list[torch.Tensor] = []
    influence_weight_chunks: list[torch.Tensor] = []
    raw_influence_weight_chunks: list[torch.Tensor] = []
    lower_clipfrac_chunks: list[torch.Tensor] = []
    influence_clipfrac_chunks: list[torch.Tensor] = []

    for i, (ppo_kl, advantages, loss_mask, total_len, response_len) in enumerate(
        zip(local_ppo_kl, local_advantages, loss_masks, total_lengths, response_lengths, strict=False)
    ):
        max_seq_len = max_seq_lens[i] if max_seq_lens is not None else None
        if cp_size > 1:
            from slime.backends.megatron_utils.cp_utils import all_gather_with_cp, slice_log_prob_with_cp

            full_ppo_kl = all_gather_with_cp(ppo_kl, total_len, response_len)
            full_advantages = all_gather_with_cp(advantages, total_len, response_len)
        else:
            full_ppo_kl = ppo_kl
            full_advantages = advantages

        negative_approx_kl = torch.clamp(-full_ppo_kl, min=-20.0, max=20.0)
        full_ratio = negative_approx_kl.exp()
        threshold = torch.log(torch.as_tensor(eps_clip_c, dtype=negative_approx_kl.dtype, device=negative_approx_kl.device))
        participation_mask = (negative_approx_kl <= threshold) & loss_mask.bool()
        future_kl = _discounted_future_sum(
            negative_approx_kl * participation_mask.to(negative_approx_kl.dtype),
            participation_mask,
            gamma=gamma,
            start=future_kl_start,
            window=future_kl_window,
            average=future_kl_average,
        )

        raw_influence_weights = future_kl.exp()
        if future_kl_clip_ratio > 0.0:
            ...
            influence_weights = torch.clamp(raw_influence_weights, min=lower_bound, max=upper_bound).detach()
        else:
            upper_bound = 10.0
            lower_bound = 0.0
            influence_weights = torch.clamp(raw_influence_weights, max=upper_bound).detach()

        if future_kl_safety_threshold > 0.0:
            neg_high_is = (full_advantages < 0) & (full_ratio > future_kl_safety_threshold)
        ...
        if cp_size > 1:
            influence_weights = slice_log_prob_with_cp(
                influence_weights, total_len, response_len, qkv_format, max_seq_len
            )
            raw_influence_weights = slice_log_prob_with_cp(
                raw_influence_weights, total_len, response_len, qkv_format, max_seq_len
            )
            future_kl = slice_log_prob_with_cp(future_kl, total_len, response_len, qkv_format, max_seq_len)

        ratio = (-ppo_kl).exp()
        weighted_advantages = advantages * influence_weights
        pg_losses1 = -weighted_advantages * ratio
        pg_losses2 = -weighted_advantages * ratio.clamp(1 - eps_clip, 1 + eps_clip_high)
        clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
        pg_clipfrac = torch.gt(pg_losses2, pg_losses1).float()

        pg_losses3 = -weighted_advantages * eps_clip_c
        clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
        pg_losses = torch.where(weighted_advantages < 0, clip_pg_losses2, clip_pg_losses1)
        lower_clipfrac = torch.gt(clip_pg_losses1, pg_losses3).float() * (advantages < 0).float()

        influence_clipfrac = (
            (influence_weights >= upper_bound - 1e-7) | (influence_weights <= lower_bound + 1e-7)
        ).float()

        pg_loss_chunks.append(pg_losses)
        pg_clipfrac_chunks.append(pg_clipfrac)
        future_kl_chunks.append(future_kl)
        influence_weight_chunks.append(influence_weights)
        raw_influence_weight_chunks.append(raw_influence_weights)
        lower_clipfrac_chunks.append(lower_clipfrac)
        influence_clipfrac_chunks.append(influence_clipfrac)

    pg_loss = torch.cat(pg_loss_chunks, dim=0)
    pg_clipfrac = torch.cat(pg_clipfrac_chunks, dim=0)
    metrics = {
        "future_kl": torch.cat(future_kl_chunks, dim=0),
        "future_kl_influence_weight": torch.cat(influence_weight_chunks, dim=0),
        "future_kl_raw_influence_weight": torch.cat(raw_influence_weight_chunks, dim=0),
        "future_kl_dual_clipfrac": torch.cat(lower_clipfrac_chunks, dim=0),
        "future_kl_influence_clipfrac": torch.cat(influence_clipfrac_chunks, dim=0),
    }

    return pg_loss, pg_clipfrac, torch.cat(local_ppo_kl, dim=0), metrics
