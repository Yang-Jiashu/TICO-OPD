    ...
    def add_slime_arguments(parser):
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                "--policy-loss-type",
                type=str,
                choices=["vanilla", "future_kl"],
                default="vanilla",
                help=(
                    "Policy gradient loss variant. `vanilla` keeps the existing PPO surrogate. "
                    "`future_kl` enables the FIPO future-KL influenced PPO objective."
                ),
            )
            parser.add_argument(
                ...
                type=float,
                default=32.0,
                help="FIPO future KL exponential decay half-life. Larger values make farther future tokens matter more.",
            )
            parser.add_argument(
                "--future-kl-start",
                type=str,
                choices=["include_current", "exclude_current"],
                default="include_current",
                help="Whether the FIPO future KL sum starts at the current token or the next token.",
            )
            parser.add_argument(
                "--future-kl-window",
                type=int,
                default=-1,
                help="Maximum number of future tokens included in FIPO future KL. -1 means the full suffix.",
            )
            parser.add_argument(
                "--future-kl-average",
                action="store_true",
                default=False,
                help="Average the discounted future KL by its effective window size instead of using the sum.",
            )
            parser.add_argument(
                "--future-kl-clip-ratio",
                type=float,
                default=0.2,
                help="Clip ratio for FIPO influence weights. 0 disables symmetric/high-only clipping and only caps at 10.",
            )
            parser.add_argument(
                "--future-kl-clip-high-only",
                action=argparse.BooleanOptionalAction,
                default=True,
                help="Clip FIPO influence weights only above 1.0, matching the released FIPO recipe.",
            )
            parser.add_argument(
                "--future-kl-safety-threshold",
                type=float,
                default=10.0,
                help="Cap influence weights for negative-advantage tokens whose importance ratio exceeds this threshold. 0 disables this guard.",
            )
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Enable compression-aware OPD shaping. This preserves high-importance trajectory tokens "
                    "and penalizes low-importance continuation tokens beyond the length/coverage budget."
                ),
            )
            parser.add_argument(
                ...
                type=int,
                default=-1,
                help="Absolute token budget for compression-aware OPD. -1 disables the absolute budget.",
            )
            parser.add_argument(
                ...
                default=0.0,
                help=(
                    "Relative budget as a fraction of the sampled response length when no absolute budget is set. "
                    "For example 0.7 starts compression pressure after 70%% of the response."
                ),
            )
            parser.add_argument(
                ...
                type=float,
                default=0.0,
                help="Per-token advantage penalty coefficient for low-importance continuation tokens.",
            )
            parser.add_argument(
                "--compression-eos-coef",
                type=float,
                default=0.0,
                help="EOS/last-token shaping coefficient once importance coverage is high; truncated samples get the negative value.",
            )
            parser.add_argument(
                "--compression-coverage-threshold",
                type=float,
                default=0.9,
                help="Cumulative importance threshold after which continuation tokens are treated as compressible.",
            )
            parser.add_argument(
                "--compression-min-response-len",
                type=int,
                default=0,
                help="Do not apply compression pressure before this response-token position.",
            )
            parser.add_argument(
                "--compression-importance-decay-rate",
                type=float,
                default=32.0,
                help="Decay half-life for future accumulated token importance used by compression-aware OPD.",
            )
            parser.add_argument(
                "--compression-importance-start",
                type=str,
                choices=["include_current", "exclude_current"],
                default="include_current",
                help="Whether compression importance starts from the current token or only future tokens.",
            )
            parser.add_argument(
                "--compression-importance-window",
                type=int,
                default=-1,
                help="Future window for compression importance. -1 means full suffix.",
            )
            parser.add_argument(
                "--compression-importance-average",
                action="store_true",
                default=False,
                help="Average future importance by effective window size instead of summing.",
            )
            parser.add_argument(
                "--compression-importance-temperature",
                type=float,
                default=1.0,
                help="Temperature for normalized compression importance. Higher values flatten the importance distribution.",
            )
            parser.add_argument(
                "--compression-reward-coef",
                type=float,
                default=0.0,
                help="Rollout post-process reward penalty coefficient for exceeding the compression budget.",
            )
            parser.add_argument(
                "--compression-mask-low-importance-tokens",
                action="store_true",
                default=False,
                help="In OPD reward post-process, set loss_mask=0 for low-importance tokens in the compressible zone.",
            )
            parser.add_argument(
                "--compression-low-importance-threshold",
                type=float,
                default=0.2,
                help="Normalized importance threshold used by rollout low-importance masking.",
            )
            parser.add_argument(
...
def slime_validate_args(args):
    ...
    if args.policy_loss_type == "future_kl":
        assert (
            args.advantage_estimator != "gspo"
        ), "`--policy-loss-type future_kl` uses token-level policy ratios and is not compatible with `--advantage-estimator gspo`."
        assert args.future_kl_decay_rate > 0, "`--future-kl-decay-rate` must be positive."
        assert args.future_kl_window == -1 or args.future_kl_window > 0, "`--future-kl-window` must be -1 or positive."
        assert args.future_kl_clip_ratio >= 0, "`--future-kl-clip-ratio` must be non-negative."
        if args.eps_clip_c is None:
            args.eps_clip_c = 3.0
    if args.use_compression_opd:
        assert args.compression_length_budget == -1 or args.compression_length_budget > 0, (
            "`--compression-length-budget` must be -1 or positive."
        )
        assert args.compression_length_budget_ratio >= 0, "`--compression-length-budget-ratio` must be non-negative."
        assert args.compression_advantage_coef >= 0, "`--compression-advantage-coef` must be non-negative."
        assert args.compression_eos_coef >= 0, "`--compression-eos-coef` must be non-negative."
        assert args.compression_reward_coef >= 0, "`--compression-reward-coef` must be non-negative."
        assert 0 <= args.compression_coverage_threshold <= 1, (
            "`--compression-coverage-threshold` must be in [0, 1]."
        )
        assert args.compression_min_response_len >= 0, "`--compression-min-response-len` must be non-negative."
        assert args.compression_importance_decay_rate > 0, "`--compression-importance-decay-rate` must be positive."
        assert args.compression_importance_window == -1 or args.compression_importance_window > 0, (
            "`--compression-importance-window` must be -1 or positive."
        )
        assert args.compression_importance_temperature > 0, (
            "`--compression-importance-temperature` must be positive."
        )
        assert 0 <= args.compression_low_importance_threshold <= 1, (
            "`--compression-low-importance-threshold` must be in [0, 1]."
        )
